...
def get_args():
    ...
    parser.add_argument("--image_dir_zao", type=str, default='./dataset/new_data/train/zao',
                        help="train image dir")
    parser.add_argument("--image_dir_yuan", type=str, default='./dataset/new_data/train/yuan',
                        help="train image dir")
    parser.add_argument("--test_dir_zao", type=str, default='./dataset/new_data/train/val_zao',
                        help="test image dir")
    parser.add_argument("--test_dir_yuan", type=str, default='./dataset/new_data/train/val_yuan',
                        help="test image dir")
    ...
    parser.add_argument("--output_path", type=str, default="impulse_clean",
                        help="checkpoint dir")
    parser.add_argument("--source_noise_model", type=str, default="impulse,3,95",
    ...
    parser.add_argument("--val_noise_model", type=str, default="impulse,1,70",
...
def main():
    args = get_args()
    image_dir_zao = args.image_dir_zao
    image_dir_yuan = args.image_dir_yuan
    test_dir_zao = args.test_dir_zao
    test_dir_yuan = args.test_dir_yuan
    ...
    generator = NoisyImageGenerator(image_dir_zao, image_dir_yuan, source_noise_model, target_noise_model, batch_size=batch_size,
                                    image_size=image_size)
    val_generator = ValGenerator(test_dir_zao, test_dir_yuan, val_noise_model)
    output_path.mkdir(parents=True, exist_ok=True)
    callbacks.append(LearningRateScheduler(schedule=Schedule(nb_epochs, lr)))
    callbacks.append(TensorBoard(log_dir='./log', histogram_freq=0, batch_size=8, write_images=True))
